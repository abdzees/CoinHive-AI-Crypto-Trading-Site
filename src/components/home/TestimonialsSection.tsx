...
export const TestimonialsSection: React.FC = () => {
  ...
  return <section id="testimonials" className="py-20 my-[10px]">
      <div className="container mx-auto px-4">
        <ScrollReveal>
          <SectionHeading title="What Our Users Say" subtitle="Don't just take our word for it - hear from the traders who've transformed their strategies with CoinHive." centered />
        </ScrollReveal>
        ...
        <ScrollReveal className="mt-12 relative">
          ...
          <div className="flex justify-center mt-8 gap-2">
            <button onClick={handlePrev} className="p-2 rounded-full border border-border hover:border-gold transition-colors" aria-label="Previous testimonial">
              <ChevronLeft size={20} />
            </button>
            {testimonials.map((_, index) => <button key={index} onClick={() => {
            setAutoplay(false);
            setCurrentIndex(index);
          }} className={`w-2 h-2 rounded-full mx-1 ${currentIndex === index ? 'bg-gold' : 'bg-gray-300'}`} aria-label={`Go to testimonial ${index + 1}`} />)}
            <button onClick={handleNext} className="p-2 rounded-full border border-border hover:border-gold transition-colors" aria-label="Next testimonial">
